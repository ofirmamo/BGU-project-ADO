...
from app import db
from app.logger import log_manager, user_logger, post_logger, user_information_logger
...
def inject_user(request):
    total_time = counters.user_table_max_transaction
    user_logger.info('{} - {} - time: {} - injected'.format(request.remote_addr, request.method, str(total_time)))
...
def inject_post(requset):
    total_time = counters.post_table_max_transaction
